...
func (d *delivery) Register(ctx echo.Context) error {
	...
	data, err := d.Usecase.Buyer.Register(body)
	if err != nil {
...
func (d *delivery) Login(ctx echo.Context) error {
	...
	result, err := d.Usecase.Buyer.Login(body.Email, body.Password)
	if err != nil {
		return res.ErrorResponse(err).Send(ctx)
	}

	token, err := d.Usecase.Buyer.GenerateToken(result.ID.String())
	if err != nil {
		return res.ErrorResponse(err).Send(ctx)
	}
